...
**`cvcli`提供了一个命令行的剪贴板键值对存储工具**，由`lua`实现，并通过`luastatic`编译为`Windows`、`Linux`和`macOS`的可执行文件。
...
- **Linux/macOS**:  
  - 从`release`中下载对应平台的压缩包并解压  
  - 将可执行文件添加到系统路径中（如`/usr/local/bin`）  
  - 确保文件具有可执行权限（`chmod +x cvcli`）  
